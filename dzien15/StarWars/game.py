...
EXHAUST_PORT_WIDTH_M = TRENCH_WIDTH_M / 3.0  # ! changed by OPi to run also in Python 2
...
NEAR_PLANE_M = 0.1
FAR_PLANE_M = 180
SCALE_WIDTH = CANVAS_WIDTH / 2.0  # ! changed by OPi to run also in Python 2
SCALE_HEIGHT = CANVAS_HEIGHT / 2.0  # ! changed by OPi to run also in Python 2

FORWARD_VELOCITY_MS = 60
PROTON_TORPEDO_VELOCITY_MS = FORWARD_VELOCITY_MS * 1.1
VELOCITY_MAX_MS = 15
VELOCITY_DAMPEN = 0.9
ACCELERATION_MSS = 100
...
BLOCK_VERTEX = (
    (0, 1), (1, 2), (2, 3), (3, 0), (0, 4), (1, 5), (2, 6), (3, 7), (4, 5), (5, 6), (6, 7), (7, 4), (0, 2), (1, 3))
...
def create_barriers():
    ...
    while s < limit:
        ...
        empty_blocks = 1 - (float(s) / limit)  # ! changed by OPi to run also in Python 2
...
def init_game():
    global game_mode, pos, vel, acc, pt_pos, pt_launch_position, reached_launch_position, \
        current_barrier_index, dead, explosion_countdown
    game_mode = MODE_GAME
    pos = [0, 0, 0]
...
def is_close_to_launch_position():
    return math.fabs(get_distance_to_launch_position()) < 100  # ! changed by OPi to run also in Python
...
def project(point):
    ...
    x = float(point[0] - pos[0]) / (distance + NEAR_PLANE_M)  # ! changed by OPi to run also in Python 2
    y = float(point[1] - pos[1]) / (distance + NEAR_PLANE_M)  # ! changed by OPi to run also in Python 2
    x *= SCALE_WIDTH
    y *= SCALE_HEIGHT
    x += (CANVAS_WIDTH // 2)
    y += (CANVAS_HEIGHT // 2)
...
# Displays the Death sequence. If the flashing colours are enabled,
# then a red rectangle is drawn onto the screen every other frame.
def render_death(canvas):
    global message_tick
    if dead:
        if violent_death and (message_tick % 2 == 0):
            canvas.draw_polygon(((0, 0), (CANVAS_WIDTH, 0), (CANVAS_WIDTH, CANVAS_HEIGHT), (0, CANVAS_HEIGHT)), 1,
                                "Red", "Red")
        message_tick += 1
...
def render_barrier(canvas, barrier):
    ...
    w = TRENCH_WIDTH_M / 3.0  # Block Width  # ! changed by OPi to run also in Python 2
    h = TRENCH_HEIGHT_M / 3.0  # Block Height  # ! changed by OPi to run also in Python 2
    hw = w / 2.0  # Block Half Width  # ! changed by OPi to run also in Python 2
    hh = h / 2.0  # Block Half Height  # ! changed by OPi to run also in Python 2
    ...
    distance = 1 - 0.9 * (n - pos[2]) / FAR_PLANE_M
...
def render_exhaust_port(canvas):
    if reached_launch_position:
        y = TRENCH_HEIGHT_M / 2.0  # ! changed by OPi to run also in Python 2
        z = EXHAUST_PORT_POSITION_M
        w = EXHAUST_PORT_WIDTH_M
        hw = w / 2.0  # ! changed by OPi to run also in Python 2
...
def move_ship():
    ...
    for i in range(0, 3):
        pos[i] += float(vel[i]) / factor  # ! changed by OPi to run also in Python 2
        if acc[i] != 0:
            vel[i] += float(acc[i]) / factor  # ! changed by OPi to run also in Python 2
    ...
    if len(pt_pos) > 0:
        ...
        for p in pt_pos:
            ...
            if p[2] - pt_launch_position >= PROTON_TORPEDO_RANGE_M:
                ...
                p[2] += float(PROTON_TORPEDO_VELOCITY_MS) / fps  # ! changed by OPi to run also in Python 2

            # Check if the torpedo has hit the floor of the trench
            if p[1] > TRENCH_HEIGHT_M / 2.0:  # ! changed by OPi to run also in Python 2
                hw = EXHAUST_PORT_WIDTH_M / 2.0  # ! changed by OPi to run also in Python 2
                z = EXHAUST_PORT_POSITION_M
                ex1 = -hw
                ex2 = hw
                ez1 = z - hw
...
def constrain_ship():
    ...
    m = SHIP_WIDTH_M / 2.0  # ! changed by OPi to run also in Python 2
    if pos[0] < (-tw + m):
        pos[0] = -tw + m
    elif pos[0] > (tw - m):
        pos[0] = tw - m
    ...
    m = SHIP_HEIGHT_M / 2.0  # ! changed by OPi to run also in Python 2
    if pos[1] < (
                -th + m) and pt_launch_position < 0:  # Allow the ship to leave the trench after it has launched the torpedoes
        pos[1] = -th + m
    elif pos[1] > (th - m):
        pos[1] = th - m
...
def check_for_collisions():
    ...
    if current_barrier_index < len(barriers):
        ...
        if pos[2] > barrier[0] and pos[2] < barrier[0] + barrier[1]:
            ...
            x1 = pos[0] - SHIP_WIDTH_M / 2.0  # ! changed by OPi to run also in Python 2
            x2 = x1 + SHIP_WIDTH_M
            y1 = pos[1] - SHIP_HEIGHT_M / 2.0  # ! changed by OPi to run also in Python 2
            y2 = y1 + SHIP_HEIGHT_M

            # Calculate block size
            bw = TRENCH_WIDTH_M / 3.0  # ! changed by OPi to run also in Python 2
            bh = TRENCH_HEIGHT_M / 3.0  # ! changed by OPi to run also in Python 2
            bhw = bw / 2.0  # ! changed by OPi to run also in Python 2
            bhh = bh / 2.0  # ! changed by OPi to run also in Python 2
            for by in range(-1, 2):
                by1 = by * bh - bhh
                by2 = by1 + bh
...
def render_deathstar(canvas, fill_colour=None):
    ...
    if fill_colour == None:
        ...
        canvas.draw_circle((centre[0] - radius * 0.35, centre[1] - radius * 0.5), radius * 0.2, LINE_WIDTH,
                           DEATH_STAR_COLOUR)
        canvas.draw_line((centre[0] - radius, centre[1] - 3), (centre[0] + radius, centre[1] - 3), LINE_WIDTH,
                         DEATH_STAR_COLOUR)
        canvas.draw_line((centre[0] - radius, centre[1] + 3), (centre[0] + radius, centre[1] + 3), LINE_WIDTH,
                         DEATH_STAR_COLOUR)
    else:
...
def render_intro_text(canvas):
    ...
    draw_text_centre(canvas,
                     "Thanks to Joe, Scott, John, Stephen & Rice University for providing a fantastic 'Introduction to Python' Course",
                     580, 16, INTRO_TEXT_COLOUR)
    ...
    draw_text_centre(canvas, "Press 'Q' to turn " + ("OFF" if violent_death else "ON") + " flashing colours", 520, 18,
                     WARNING_TEXT_COLOUR)
    if violent_death:
        draw_text_centre(canvas,
                         "Note: this game contains flashing colours which are not suitable for sufferers of epilepsy",
                         500, 18, WARNING_TEXT_COLOUR)
...
def render_victory(canvas):
    ...
    if explosion_countdown <= 0:
        if explosion_countdown > -160:
            ...
            factor = -explosion_countdown / 10.0  # ! changed by OPi to run also in Python 2
            colour = "#"
            for c in range(0, 3):
